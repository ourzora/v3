...
library LibReserveAuctionV1 {
    ...
    function createBid(
        ...
        require(
            _amount >= _self.auctions[_auctionId].reservePrice,
            "createBid must send at least reservePrice"
        );
        require(
            _amount >=
        ...
        auctionExists(_self, _auctionId)
    {
        require(
            _self.auctions[_auctionId].firstBidTime != 0,
            "endAuction auction hasn't begun"
        );
        require(
            block.timestamp >=
                _self.auctions[_auctionId].firstBidTime.add(
            ...
            "endAuction auction hasn't completed"
        );
