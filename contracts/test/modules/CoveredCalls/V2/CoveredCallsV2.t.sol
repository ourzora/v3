...
import {CoveredCallsV2} from "../../../../modules/CoveredCalls/V2/CoveredCallsV2.sol";
...
/// @title CoveredCallsV2Test
/// @notice Unit Tests for Covered Call Options v2.0
contract CoveredCallsV2Test is DSTest {
    VM internal vm;
    ...
    function setUp() public {
        ...
        // Init V3
        registrar.init(ZMM);
        ZPFS.init(address(ZMM), address(weth));
        ...
        // Deploy Covered Call Options v2.0
        calls = new CoveredCallsV2(address(erc20TransferHelper), address(erc721TransferHelper), address(royaltyEngine), address(ZPFS), address(weth));
    ...
    function test_CreateCall() public {
        ...
        require(callCurrency == address(weth));
    ...
    function test_CreateCallAndCancelInactiveCall() public {
        ...
        ZMM.setApprovalForModule(address(calls), true);
        token.setApprovalForAll(address(erc721TransferHelper), true);
        calls.createCall(address(token), 0, 2 ether, 10 ether, 3 days, address(weth));
    ...
    function testRevert_CreateCallExpirationMustBeFutureTime() public {
        ...
        calls.createCall(address(token), 0, 0.5 ether, 1 ether, 0, address(weth));
    }
    ...
    function testRevert_CannotCancelPurchasedCall() public {
        ...
        vm.prank(address(buyer));
        calls.buyCall{value: 0.5 ether}(address(token), 0, address(weth), 0.5 ether, 1 ether);
