...
contract CoveredCallsV1Test is DSTest {
    ...
    function test_CreateCall() public {
        ...
        (address callSeller, address callBuyer, uint256 callPremium, uint256 callStrike, uint256 callExpiry) = calls.callForNFT(address(token), 0);
    ...
    function test_CreateCallAndCancelInactiveCall() public {
        ...
        ZMM.setApprovalForModule(address(calls), true);
        token.setApprovalForAll(address(erc721TransferHelper), true);
        calls.createCall(address(token), 0, 2 ether, 10 ether, 3 days);
    ...
    function testRevert_CreateCallExpirationMustBeFutureTime() public {
        ...
        calls.createCall(address(token), 0, 0.5 ether, 1 ether, 0);
    }
    ...
    function testRevert_BuyCallDoesNotExist() public {
        ...
        calls.buyCall(address(token), 0, 0.5 ether, 1 ether);
    }
