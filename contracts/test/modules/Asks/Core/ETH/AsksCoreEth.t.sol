...
import {AsksCoreEth} from "../../../../../modules/Asks/Core/ETH/AsksCoreEth.sol";
...
/// @title AsksCoreETHTest
/// @notice Unit Tests for Asks Core ETH
contract AsksCoreEthTest is DSTest {
    VM internal vm;
    ...
    function setUp() public {
        ...
        // Deploy Asks Core ETH
        asks = new AsksCoreEth(address(erc721TransferHelper), address(royaltyEngine), address(ZPFS), address(weth));
    ...
    function test_CreateAskFromTokenOperator() public {
        ...
        asks.createAsk(address(token), 0, 0.5 ether);
    ...
    function test_CreateAskAndOverridePrevious() public {
        ...
        vm.prank(address(otherSeller));
        asks.createAsk(address(token), 0, 10 ether);
    ...
    function test_CreateMaxAskPrice() public {
        vm.prank(address(seller));
        asks.createAsk(address(token), 0, 2**96 - 1);
