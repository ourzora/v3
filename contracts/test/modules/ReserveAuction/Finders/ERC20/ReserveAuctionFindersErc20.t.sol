...
contract ReserveAuctionFindersErc20Test is DSTest {
    VM internal vm;
    ...
    function test_CreateFutureAuction() public {
        vm.prank(address(seller));
        auctions.createAuction(address(token), 0, 1 days, 1 ether, address(sellerFundsRecipient), 1 days, address(weth), 1000);
    ...
    function testRevert_FindersFeeBPSCannotExceed10000() public {
        ...
        auctions.createAuction(address(token), 0, 1 days, 1 ether, address(sellerFundsRecipient), 0, address(weth), 10001);
    }
    ...
    function testRevert_StartTimeMustBeValid() public {
        ...
        auctions.createAuction(address(token), 0, 1 days, 1 ether, address(sellerFundsRecipient), 5 hours, address(weth), 1000);
    }
    ...
    function test_ExtendAuction() public {
        vm.prank(address(seller));
        auctions.createAuction(address(token), 0, 1 hours, 1 ether, address(sellerFundsRecipient), 0, address(weth), 1000);
    ...
    function testRevert_CannotBidOnExpiredAuction() public {
        vm.prank(address(seller));
        auctions.createAuction(address(token), 0, 10 hours, 1 ether, address(sellerFundsRecipient), 0, address(weth), 1000);
