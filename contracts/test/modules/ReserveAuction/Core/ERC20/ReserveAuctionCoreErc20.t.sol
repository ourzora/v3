...
/// @title ReserveAuctionCoreErc20Test
/// @notice Unit Tests for Reserve Auction Core ERC-20
contract ReserveAuctionCoreErc20Test is DSTest {
    VM internal vm;
    ...
    function testGas_CreateERC20Auction() public {
        vm.prank(address(seller));
        auctions.createAuction(address(token), 0, 1 days, 1 ether, address(sellerFundsRecipient), 0, address(weth));
    }

    function test_CreateInstantERC20Auction() public {
        vm.prank(address(seller));
        auctions.createAuction(address(token), 0, 1 days, 1 ether, address(sellerFundsRecipient), 0, address(weth));
    ...
    function test_CreateFutureERC20Auction() public {
        vm.prank(address(seller));
        auctions.createAuction(address(token), 0, 1 days, 1 ether, address(sellerFundsRecipient), 1 days, address(weth));
        (, , , , , , uint256 startTime, , ) = auctions.auctionForNFT(address(token), 0);

        require(startTime == 1 days);
    }

    function test_CreateERC20AuctionAndCancelPrevious() public {
        vm.prank(address(seller));
        auctions.createAuction(address(token), 0, 1 days, 1 ether, address(sellerFundsRecipient), 0, address(weth));
        ...
        token.setApprovalForAll(address(erc721TransferHelper), true);
        auctions.createAuction(address(token), 0, 5 days, 12 ether, address(sellerFundsRecipient), 0, address(weth));
        vm.stopPrank();
    ...
    function testRevert_MustBeValidReservePrice() public {
        ...
        auctions.createAuction(address(token), 0, 1 days, reservePrice, address(sellerFundsRecipient), 0, address(weth));
    }

    function testRevert_MustSpecifySellerFundsRecipient() public {
        vm.prank(address(seller));
        vm.expectRevert("INVALID_FUNDS_RECIPIENT");
        auctions.createAuction(address(token), 0, 1 days, 1 ether, address(0), 0, address(weth));
    }
    ...
    function test_ExtendAuction() public {
        vm.prank(address(seller));
        auctions.createAuction(address(token), 0, 1 hours, 1 ether, address(sellerFundsRecipient), 0, address(weth));
    ...
    function testRevert_CannotBidOnExpiredAuction() public {
        vm.prank(address(seller));
        auctions.createAuction(address(token), 0, 5 days, 1 ether, address(sellerFundsRecipient), 0, address(weth));
    ...
    function testRevert_MaxBid() public {
        vm.prank(address(seller));
        auctions.createAuction(address(token), 0, 1 days, 1 ether, address(sellerFundsRecipient), 0, address(erc20));
