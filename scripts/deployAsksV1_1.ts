...
export async function deployAsksV1_1(_, hre: HardhatRuntimeEnvironment) {
  // @ts-ignore
  ...
  console.log(`Deploying AsksV1_1 from address ${await deployer.getAddress()}`);

  // @ts-ignore
  const AsksFactory = await hre.ethers.getContractFactory('AsksV1_1');
  ...
  console.log(
    `Deploying AsksV1_1 with tx ${asks.deployTransaction.hash} to address ${asks.address}`
  );

  await asks.deployed();
  addressBook.AsksV1_1 = asks.address;
  await fs.writeFile(addressPath, JSON.stringify(addressBook, null, 2));
  console.log(`Deployed AsksV1_1`);
}
